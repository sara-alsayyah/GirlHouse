        ...
        <section className="mt-8 space-y-6">
            ...
            <div className="grid gap-8 lg:grid-cols-[1fr,0.95fr]">
              <div className="luxury-card rounded-[34px] p-6">
                ...
                <div className="mt-5 grid gap-4 sm:grid-cols-2">
                  <div className="rounded-[22px] border border-[rgba(166,122,122,0.14)] bg-white/66 p-4">
              ...
              <div className="luxury-card rounded-[34px] p-6">
                ...
                <div className="mt-5 space-y-4">
                  {orders.slice(0, 3).map((order) => (
                    <div key={order.id} className="rounded-[22px] border border-[rgba(166,122,122,0.14)] bg-white/66 p-4">
          ...
          {activeTab === "profile" ? (
            <div className="luxury-card rounded-[34px] p-6">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                <input value={profile?.first_name ?? ""} onChange={(event) => setProfile((current) => (current ? { ...current, first_name: event.target.value } : current))} placeholder="First name" className="rounded-[18px] border border-[rgba(166,122,122,0.16)] bg-white/72 px-4 py-3 outline-none" />
                <input value={profile?.last_name ?? ""} onChange={(event) => setProfile((current) => (current ? { ...current, last_name: event.target.value } : current))} placeholder="Last name" className="rounded-[18px] border border-[rgba(166,122,122,0.16)] bg-white/72 px-4 py-3 outline-none" />
                <input value={profile?.email ?? ""} onChange={(event) => setProfile((current) => (current ? { ...current, email: event.target.value } : current))} placeholder="Email" className="rounded-[18px] border border-[rgba(166,122,122,0.16)] bg-white/72 px-4 py-3 outline-none sm:col-span-2" />
                <input value={profile?.phone ?? ""} onChange={(event) => setProfile((current) => (current ? { ...current, phone: event.target.value } : current))} placeholder="Phone" className="rounded-[18px] border border-[rgba(166,122,122,0.16)] bg-white/72 px-4 py-3 outline-none sm:col-span-2" />
              </div>
            ...
            <div className="luxury-card rounded-[34px] p-6">
              ...
              <div className="mt-5 space-y-4">
                {addresses.map((address) => (
                  <div key={address.id} className="rounded-[24px] border border-[rgba(166,122,122,0.14)] bg-white/66 p-4">
                    <div className="flex items-center justify-between gap-4">
          ...
          {activeTab === "orders" ? (
            <div className="luxury-card rounded-[34px] p-6">
              ...
              <div className="mt-5 space-y-4">
                {!orders.length ? (
                  <div className="flex flex-col items-center justify-center rounded-[24px] border border-dashed border-[rgba(166,122,122,0.2)] bg-white/50 p-8 text-center">
                          ...
                          {steps.map((step, index) => {
                            ...
                            return (
                              <div key={step} className="flex flex-col items-center gap-1">
                                <span className={`h-3 w-3 rounded-full ${active ? "bg-[var(--gold)]" : "bg-[rgba(166,122,122,0.18)]"}`} />
